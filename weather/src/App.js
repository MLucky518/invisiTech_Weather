...
function App() {
  const getWeatherByZip = (input) => {
    axios
      .get(
        `http://api.openweathermap.org/data/2.5/weather?zip=${input},us&appid=${process.env.REACT_APP_API_KEY}`
      )
      .then((res) => {
  ...
  const getWeatherByName = (input) => {
    axios
      .get(
        `http://api.openweathermap.org/data/2.5/weather?q=${input},us&appid=${process.env.REACT_APP_API_KEY}`
      )
      .then((res) => {
